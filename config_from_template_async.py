...
TEMPLATE_FILE = "templates/template_snmp.j2"
...
class DeviceConfigure:
    ...
    def get_template_configs(self, file_name=TEMPLATE_FILE):
    ...
    async def configure_device(self, ip_address):
        ...
        try:
            async with netdev.create(**device_params) as device_conn:
                ...
                print("Deploying configs to {hostname}".format(**parsed_values))

                commands_output = [
                    "Configs deployed to {hostname}".format(**parsed_values)
                ]

                template_configs = self.get_template_configs()
